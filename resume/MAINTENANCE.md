...
- Deeplink caching (user-confirmed correction): DynamoDB is the persistent source of deeplink data. TTL/expiration applies to application-server Local Cache entries, not DynamoDB items or link lifecycle management. Valid cache entries serve reads without a DynamoDB query; after expiration, the next request reloads the value from DynamoDB and refreshes the local cache. Retain the approximately 30% reduction in DynamoDB access, duplicate prevention, and administrative search/deletion. Do not claim automatic DynamoDB record deletion or add an unconfirmed caching library or TTL duration.
- AnyID / SSO (user-confirmed correction): provided technical guidance on integration, not personal implementation. Keep guidance distinct from the public-sector features the user developed. Do not infer AnyID coding, end-to-end design, testing, or delivery ownership from the team-lead title. Apply this distinction to the English resume and Cloudflare submission page.
