...
포인트 지급 정책에서는 **현재 회원등급 조회와 과거 주문의 거래 조건 재현을 분리**했습니다. 현재 상태는 `member → member_tier`를 유기적으로 조회하되, 과거 주문은 당시 등급뿐 아니라 당시 등급의 포인트 정책까지 재현할 수 있어야 하므로 **주문 Snapshot 또는 Tier Policy Versioning이 필요**하다고 정리했습니다. 최종 지급 포인트는 수량 변경·취소·회수·재지급 가능성을 고려해 지급 시점에 다시 계산하는 방향으로 개발리더와 결정했습니다.

**AI가 구현 속도를 높일수록, 무엇이 올바른 구현인지 판단할 수 있는 도메인 이해와 설계 기준이 더 중요해진다고 생각합니다.** AI는 코드를 만들 수 있지만 데이터의 의미와 경계, 정책 간 충돌, 운영에서 생길 예외를 스스로 결정해주지는 않습니다. 그래서 AI를 구현 도구로 적극 활용하되, 사람이 먼저 도메인 규칙과 검증 기준을 정의하고 생성 결과를 실제 데이터와 실행 경로로 검증하는 방식으로 개발합니다.
...
- 현재 회원 상태 조회와 과거 거래 재현을 분리하고, **주문 당시 회원등급과 당시 등급 정책을 복원할 수 있는 Snapshot / Versioning 필요성**을 정의
...
- **AI가 구현을 맡더라도 사람이 도메인 invariant, 데이터 수명주기, 경계 조건, 검증 기준을 먼저 정의해야 한다는 원칙으로 사용**
...
### 3. 현재 회원 상태와 과거 거래 재현을 분리한 포인트 정책 설계

**Java · Spring Boot · PostgreSQL · Domain Policy · Temporal Policy**

포인트 정책을 검토하면서 **현재 상태를 조회하는 문제와 과거 거래를 재현하는 문제는 서로 다른 책임**이라고 정리했습니다.

현재 마이페이지에서 회원등급을 보여주거나 다음 주문에 적용할 정책을 구할 때는 `member → member_tier`를 조회하는 것이 맞습니다. 반면 이미 발생한 주문에 대해 나중에 포인트를 지급할 때는 **주문 당시 회원등급뿐 아니라 그 시점의 등급별 포인트 정책까지 재현**할 수 있어야 합니다.
...
member_tier
   ↓
현재 receive_point_rate / receive_point_amount
```

현재 상태는 Master를 유기적으로 조회하고, 주문 Snapshot을 최신값으로 계속 갱신하지 않습니다.

#### `member_tier_history`만으로 부족한 이유

기존 회원등급 이력이 회원의 등급 변경 시점과 `before_tier_id / after_tier_id`를 보존하더라도, 이것만으로는 **당시 등급 정책 자체의 값**까지 복원할 수 없습니다.

예를 들어 주문 당시 BRONZE 정책이 `0.5% + 100P`였는데 이후 같은 BRONZE 정책이 `2% + 300P`로 바뀐다면, 회원등급 이력으로 당시 `BRONZE`였다는 사실은 알 수 있어도 현재 `member_tier`를 JOIN해서는 과거의 `0.5% + 100P`를 재현할 수 없습니다.

즉 과거 거래를 재현하려면 다음 중 하나가 필요하다고 정리했습니다.

#### 선택지 A — 주문에 정책 Snapshot

`order_item_post_delivery`에 주문 당시 값을 고정합니다.

- `member_tier_id`
- `member_tier_receive_point_rate`
- `member_tier_receive_point_amount`

Snapshot은 정책 변경 시 함께 UPDATE하는 복사본이 아니라, **정책이 변경되어도 과거 주문의 기준을 바꾸지 않기 위해 저장하는 거래 시점의 사실**입니다.

#### 선택지 B — Tier 정책 자체를 Versioning

정규화된 구조를 유지하려면 등급 정책 자체의 유효기간 이력이 필요합니다.

```text
tier_policy_history
- tier_id
- receive_point_rate
- receive_point_amount
- valid_from
- valid_to
```

이 경우 주문 시각을 기준으로 회원의 당시 Tier와 그 시점에 유효했던 Tier Policy Version을 함께 찾아야 과거 지급 조건을 재현할 수 있습니다.

```mermaid
flowchart TD
    A[Current Member View] --> B[member]
    B --> C[current member_tier]

    D[Past Order] --> E{How to reconstruct?}
    E --> F[Order Policy Snapshot]
    E --> G[Member Tier History + Tier Policy Version]
    F --> H[Original Reward Policy]
    G --> H
```

#### 최종 지급 포인트는 별도 판단

초기에는 최종 지급 예정 포인트인 `point_reward_amount`까지 저장하는 안도 검토했습니다. 이는 주문 당시 확정 금액을 그대로 보존한다는 장점이 있지만, 실제 지급 전후에는 수량 변경·일부/전체 취소·회수·재지급이 발생할 수 있습니다.

개발리더와 이 시나리오를 검토한 결과, 현재 구조에서는 **주문 당시 정책을 재현할 정보는 보존하되 최종 지급액은 지급 시점의 유효 수량·취소 상태를 반영해 다시 계산**하는 편이 정합성을 유지하기 쉽다고 판단했습니다.

이 사례에서는 Snapshot을 무조건 복제 데이터로 보거나 반대로 모든 값을 현재 Master에서 조회하는 식으로 단순화하지 않고, **현재 상태와 과거 거래 재현, 정책 원본과 파생 결과의 책임을 나눠 설계**했습니다.
...
- 전체 후보를 `random()`으로 정렬하는 방식의 상품 수 증가 시 응답 성능 리스크 식별
...
현재 회원등급처럼 최신 Master를 조회해야 하는 값과, 주문 당시 등급 정책처럼 과거 거래를 그대로 재현해야 하는 값은 같은 방식으로 다루지 않습니다. **현재 상태는 유기적으로 조회하고, 과거 거래는 Snapshot이나 Version을 통해 당시 기준을 복원할 수 있어야 한다**고 생각합니다.

### 변하지 않는 원천과 변할 수 있는 파생값을 구분합니다

주문 당시 정책처럼 나중에 바뀌면 안 되는 거래의 기준은 보존하지만, 수량·취소 상태에 따라 달라질 수 있는 최종 포인트처럼 파생된 값은 섣불리 저장하지 않습니다. **정합성을 위해 무엇을 저장하지 않을지도 설계의 일부**라고 생각합니다.
...
AI가 생성한 코드도 다른 코드와 동일하게 원천 데이터, 정책, 테스트 결과로 검증합니다. **AI가 구현을 맡을 수 있어도, 무엇이 올바른 구현인지 판단하는 책임까지 넘길 수는 없다고 생각합니다.** 실행 성공보다 올바른 결과를 만드는지가 더 중요합니다.
